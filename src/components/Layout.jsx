...
const Layout = () => {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-20 backdrop-blur-xl transition-all duration-300 ${
        scrolled 
          ...
          <div className="flex items-center justify-between h-16 min-h-[64px]">
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
              ...
              <div className="hidden sm:block">
                <h1 className={`text-base sm:text-lg font-extrabold transition-all duration-300 leading-tight ${
                  scrolled 
                    ? 'text-white drop-shadow-[0_3px_10px_rgba(0,0,0,1)]' 
                    : 'text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]'
                }`}>Gestion RH CGC</h1>
                <p className={`text-[10px] hidden lg:block transition-all duration-300 font-semibold leading-tight ${
                  scrolled 
            ...
            <div className="hidden md:flex items-center space-x-1 flex-1 justify-center mx-2 overflow-x-auto min-w-0">
              {mainNavigation.map((item, index) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`group relative flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg transition-all duration-300 hover-lift whitespace-nowrap flex-shrink-0 ${
                    isActive(item.href)
                ...
                >
                  ...
                  <span className={`text-xs sm:text-sm font-extrabold relative z-10 ${
                    isActive(item.href) 
            ...
            <div className="md:hidden relative flex-shrink-0">
              <button
                ...
                className={`flex items-center justify-center w-10 h-10 rounded-lg backdrop-blur-sm cursor-pointer transition-all duration-300 ${
                  scrolled 
                    ? 'bg-white hover:bg-gray-100 border-2 border-white shadow-[0_4px_12px_rgba(255,255,255,0.5)]' 
                    : 'bg-white/30 hover:bg-white/40 border-2 border-white/60 shadow-lg'
                }`}
              >
              ...
              {mobileMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-72 glass backdrop-blur-xl bg-gradient-to-b from-blue-700 to-blue-800 border border-blue-400/30 rounded-2xl shadow-2xl p-4 animate-fade-in-up z-50">
                      ...
                      <Link
                        ...
                        className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                          isActive(item.href)
                            ...
                            : 'bg-blue-600/40 hover:bg-blue-500/50 border border-blue-400/20'
                        }`}
                      >
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
              <div className={`hidden lg:flex items-center space-x-2 px-3 py-1.5 rounded-lg backdrop-blur-sm transition-all duration-300 cursor-pointer group ${
                scrolled 
                  ? 'bg-white hover:bg-gray-100 border-2 border-white shadow-[0_4px_12px_rgba(255,255,255,0.5)]' 
                  : 'bg-white/25 hover:bg-white/35 border-2 border-white/60 shadow-lg'
              }`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-lg transition-all duration-300 flex-shrink-0 ${
                  user?.role === 'admin' 
                ...
                <div className="flex flex-col min-w-0">
                  <p className={`text-xs font-extrabold truncate max-w-[100px] ${
                    scrolled 
                      ? 'text-gray-900 drop-shadow-[0_2px_4px_rgba(255,255,255,0.8)]' 
                      : 'text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]'
